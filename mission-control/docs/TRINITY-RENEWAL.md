...
## Third iteration · 25 September 2026

- Workspace selector: private home screen and scoped custom app links, new notes, goals, planner and idea boards. Existing Kanban/calendar/team and legacy notebook data are still shared local modules; this selector is not a tenant security boundary or a completed second cloud account.
- Obsidian folder bridge: user-selected local vault; per-workspace subfolder; stable Markdown filenames, three-way comparison against a saved baseline, explicit conflict resolution, no automatic deletes. Notes import back; edits to generated project/profile mirrors are preserved as separate notes rather than silently rewriting their source modules. Optional 30-second sync while the page is visible. Re-select folder after reload. Requires a browser implementing `showDirectoryPicker` (desktop Chrome/Edge); ZIP export remains the fallback. Pure reconciliation tested; a real user's vault has not been connected.
- Directory: shared category model for the sidebar dropdown and directory cards, multi-category assignment, configurable categories per app. GoHighLevel appears under CRM, communication, websites/design and marketing/automation. Official-site favicon assets for eight recognizable brands include source URLs in public/brands/SOURCES.json. Other tools retain letter/symbol fallbacks.
- Eight YOU ARE NEO room links from the existing app gallery, all checked HTTP 200. Gallery-hosted rooms may be earlier app versions; they are links, not new authenticated storage integrations. Hookbook was not identified independently from Cookbook.
- Idea canvas: text/media, editable tables, numeric bar charts, arithmetic formulas without eval, contact cards, right-click creation/linking, Word and PDF text/table export. Export preserves text but is not a full visual canvas/rich-media renderer. Email prepares a mail-client draft; automatic delivery and attachment upload are not wired.
- Contacts: local contacts and an authenticated, single-owner HubSpot read adapter with pagination. Requires server-side HUBSPOT_PRIVATE_APP_TOKEN plus HUBSPOT_TRINITY_USER_ID. No token configured here, no live contacts imported. The installed HubSpot connector reported unfinished onboarding; user confirmation was requested before initiating it. No CRM writes performed.
- Daily planner: date-specific top three, opportunity/reflection fields, chosen goal, 100 persistent reasons per goal in ten pages, mindmap/pyramid/progress views. Original Trinity UI inspired by the general vision/planning/reflection approach; not a reproduction of the commercial planner.
- Optional bounded AI context: current workspace goals, vision and six recent notes, controlled in Settings. Content goes to the user's selected agent provider only when enabled and chatting. No autonomous full-workspace indexing or background model loop.
- 20 tests cover the workspace, birth systems, OAuth boundaries, vault conflict decisions, arithmetic parsing and Word XML escaping. Browser verified private canvas selection, table creation/row editing, Word/PDF export actions and local persistence. Public deployment remains pending the hosting handover.

References: https://obsidian.md/help/data-storage ; https://shop.bodoschaefer-akademie.de/products/chancen-planer-2026 ; https://developers.hubspot.com/docs/api-reference/legacy/crm/objects/contacts/guide ; https://www.gohighlevel.com/
