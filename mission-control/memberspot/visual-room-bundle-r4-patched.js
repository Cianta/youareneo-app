...
function drawSky(t) {
  ...
  if (night > .02) {
    ...
    // Ruhige, seltene Sternschnuppen: ein kurzer Weg durch den realistischen Nachthimmel.
    var meteorCycle = (t % 17), meteorA = smooth((meteorCycle - 10.6) / .32) * (1 - smooth((meteorCycle - 12.1) / .5));
    if (meteorA > .01) {
      var mx0 = W * .18, my0 = H * .12, md = (meteorCycle - 10.6) * W * .13;
      var mt = ge.createLinearGradient(mx0 + md - 90, my0 + md * .42 - 38, mx0 + md, my0 + md * .42);
      mt.addColorStop(0, 'rgba(116,206,255,0)'); mt.addColorStop(.72, 'rgba(155,224,255,' + (.22 * meteorA) + ')'); mt.addColorStop(1, 'rgba(255,245,204,' + (.96 * meteorA) + ')');
      ge.strokeStyle = mt; ge.lineWidth = 1.4 + bands.high * 1.8; ge.beginPath(); ge.moveTo(mx0 + md - 100, my0 + md * .42 - 42); ge.lineTo(mx0 + md, my0 + md * .42); ge.stroke();
    }
...
function drawSpirit(t, dream, hue) {
  if (!spirit && t > nextSpirit && W > 360) {
    var pool=PIXEL?ANIMALS:ANIMALS.filter(function(a){return VR_PHOTOS[a.n]||['Wolf','Eule','Hirsch','Fuchs','Adler','Tiger'].includes(a.n);});var mp = moonPool().filter(function(a){return pool.includes(a);}), A;
    if (mp.length && Math.random() < .6) A = mp[Math.random() * mp.length | 0];
    else { var i; do { i = Math.random() * pool.length | 0; } while (i === lastAnimal && pool.length > 1); lastAnimal = i; A = pool[i]; }
    prepAnimal(A);
  ...
  gm.setLineDash([]);
  // Im Cinematic-Raum werden die Krafttiere aus Sternenlinien geboren. Die Foto-Atlasbilder
  // bleiben ausschließlich dem Pixelraum vorbehalten, damit keine rechteckigen Foto-Kanten auftauchen.
  var starBirth = drawP * (1 - eraseP), starFade = (1 - smooth((tau - 6.6) / 2.4));
  if (!PIXEL && starBirth > .01) {
    gm.globalCompositeOperation = 'lighter';
    for (var si = 0; si < 94; si++) {
      var sa = si * 2.399 + tau * (si % 3 ? .22 : -.3), sr = 60 + (si * 37 % 210) * (1 - drawP * .72);
      var sx = 256 + Math.cos(sa) * sr, sy = 256 + Math.sin(sa * 1.27) * sr * .72;
      var ss = .7 + (si % 5) * .23 + bands.high * 1.5;
      gm.globalAlpha = starBirth * starFade * (.12 + (si % 7) * .025);
      gm.fillStyle = 'hsla(' + (hue + si * 3 % 45) + ',100%,82%,1)'; gm.beginPath(); gm.arc(sx, sy, ss, 0, 6.2832); gm.fill();
    }
  }
  if (colorA > .01) {
    if (PIXEL && typeof nxAnimalAtlas !== 'undefined' && nxAnimalAtlas && ready(nxAnimalAtlas) && nxAtlasAnimals.indexOf(A.n)>=0) {var ai=nxAtlasAnimals.indexOf(A.n),aw=nxAnimalAtlas.naturalWidth/3,ah=nxAnimalAtlas.naturalHeight/2;gm.globalCompositeOperation='screen';gm.globalAlpha=colorA;gm.drawImage(nxAnimalAtlas,(ai%3)*aw,Math.floor(ai/3)*ah,aw,ah,-45,-45,602,602);}
    else if (PIXEL && A.photo && A.photo.complete && A.photo.naturalWidth) { gm.globalCompositeOperation = 'screen'; gm.globalAlpha = colorA; gm.drawImage(A.photo, -110, -110, 732, 732); }
...
var SEQ = { on: false, drumsOn: true, meter: [4, 4], steps: 16, bars: 1, grid: {}, step: 0, nextTime: 0, timer: null, bar: 0 };
function stepsFor(m) { return m[0] * (16 / m[1]); }
function resetGrid() {
  SEQ.steps = stepsFor(SEQ.meter) * (SEQ.bars || 1);
...
function patternData() {
  return { meter: SEQ.meter, bars: SEQ.bars || 1, grid: SEQ.grid, drumsOn: SEQ.drumsOn, instr: INSTR.map(function (i) { return { id: i.id, on: i.on, vol: i.vol, sample: i.sample && i.sample.id }; }), drums: DRUMS.map(function (d) { return { id: d.id, on: d.on, sample: d.sample && d.sample.id }; }) };
}
function applyPattern(p) {
  var d = p.data || {}; SEQ.meter = d.meter || [4, 4]; SEQ.bars = Math.max(1, Math.min(8, Number(d.bars) || 1)); resetGrid();
...
function setPMode(m) {
  ...
  if (window.__vrPlayerFixActive) return; // Wächter läuft schon
  window.__vrPlayerFixActive = true;
  try { localStorage.setItem('vr_pmode', 'dock'); } catch (e) {}
  var stripBad = function () {
    var pw = document.getElementById('vr-pwrap');
    if (!pw) return;
    if (pw.classList.contains('hdr')) pw.classList.remove('hdr');
    if (pw.classList.contains('mini')) pw.classList.remove('mini');
    var rr = document.getElementById('vr-root');
    if (rr && rr.classList.contains('vr-compact-player')) rr.classList.remove('vr-compact-player');
  };
  stripBad();
  if (typeof MutationObserver === 'function') {
    var mo = new MutationObserver(stripBad);
    mo.observe(pw, { attributes: true, attributeFilter: ['class'] });
    var rr2 = document.getElementById('vr-root');
    if (rr2) mo.observe(rr2, { attributes: true, attributeFilter: ['class'] });
  }
  var t = 0;
  var boot = setInterval(function () {
    if (typeof window.vrDockC === 'function') {
      var r = pw.getBoundingClientRect();
      if (r.width > 40 && r.height > 20) window.vrDockC(r.width, r.height, null);
    }
    if (++t > 12) clearInterval(boot);
  }, 120);
}
...
(function () {
  ...
  function drawCosmos(t) {
    ...
    if ((p > 0.5) !== (HEMISPHERE === 'south')) cosmosCtx.scale(-1, 1); // abnehmend → spiegeln
  ...
  // === /CINEMATIC COSMOS ==============================================
  var mandalaOriginal=drawMandala;
  drawMandala=function(t,p,f,scale,alpha,spin,fine){mandalaOriginal(t,p,f,scale,alpha*Math.max(smooth(clamp(svS/.8,0,1)),smooth(clamp((performance.now()-nxLastActive-5000)/6000,0,1))*.55),spin,fine);};
  var energyOriginal = drawEnergy;
  drawEnergy = function (t) {
    ge.setTransform(DPR,0,0,DPR,0,0); ge.globalCompositeOperation='source-over'; ge.clearRect(0,0,W,H);
    var g=ge.createRadialGradient(W*.5,H*.35,0,W*.5,H*.4,Math.max(W,H));
    g.addColorStop(0,'#11152f');g.addColorStop(.5,'#050c19');g.addColorStop(1,'#02050a');
    ge.fillStyle=g;ge.fillRect(0,0,W,H);
  };
  drawSky = function () {};
  ...
  drawSnake = function (t, u) {
    ...
    var size = Math.min(W*.27, 260), x = W - TW*.7 - size*.45, y = lerp(H*.86, H*.2, smooth(u));
  ...
  drawFerns = function (t) {
    ...
    var growth=smooth(clamp((svS-.35)/2.4,0,1))*(typeof NX!=='undefined'?NX.plants:1); if(growth<.005)return; var fh = H*.92, fw=fh*foliage.naturalWidth/foliage.naturalHeight;
    [-1,1].forEach(function (side) { gf.save(); gf.translate(side===-1 ? -fw*.2 : W+fw*.2,H); gf.scale(side===-1 ? 1 : -1,1); gf.rotate(reduced ? 0 : Math.sin(t*.22+side)*.009); gf.scale(.75+.25*growth,.25+.75*growth);gf.globalAlpha=.78*growth; gf.drawImage(foliage,0,-fh,fw,fh); gf.restore(); });
  };
  var groundOriginal = drawGround;
  drawGround = function (t) {
    if (!ready(scene)) { groundOriginal(t); return; }
    var fit=Math.min(TWd/scene.naturalWidth,THd/scene.naturalHeight),sw=scene.naturalWidth*fit,sh=scene.naturalHeight*fit;tg.fillStyle='#050c10';tg.fillRect(0,0,TWd,THd);tg.drawImage(scene,(TWd-sw)/2,(THd-sh)/2,sw,sh);
    tg.fillStyle='rgba(3,9,12,.12)';tg.fillRect(0,0,TWd,THd);
    mist(tg,TWd,THd,t,.07);
  };
...
/* Atmosphere Studio — shared by Cinematic and Pixel, no external audio dependency. */
var NX = { tab:'music', analyserMode:'spectrum', analyserSource:'all', chimes:store('vr_chimes')==='on', weather:'clear', light:.65, plants:1, animals:true, fx:{}, nodes:null, discovered:[], ambience:{fire:0,waterfall:0,river:0,rain:0,wind:0}, ambNodes:{}, kit:'frame' };
try { var savedAnimals=JSON.parse(store('vr_discovered')||'[]'); NX.discovered=Array.isArray(savedAnimals)?savedAnimals.filter(function(n){return ANIMALS.some(function(a){return a.n===n;});}):[]; } catch(e){}
function nxAsset(n){return window.VR_ASSETS&&window.VR_ASSETS[n]||assetBase+n;}
function nxRange(id,label,value,min,max,step){return '<label class="nx-control"><span>'+label+' <output>'+value+'</output></span><input aria-label="'+label+'" data-nx-range="'+id+'" type="range" min="'+min+'" max="'+max+'" step="'+step+'" value="'+value+'"></label>';}
function nxButton(id,label,on){return '<button title="'+label+'" aria-label="'+label+'" data-nx="'+id+'" aria-pressed="'+!!on+'" class="'+(on?'on':'')+'">'+label+'</button>';}
var nxChimeOriginal=chimeNote;
chimeNote=function(i,vel,dest,when){if(NX.chimes)nxChimeOriginal(i,vel,dest,when);};
var nxPlayInst=playInst;playInst=function(it,t,deg,vel){var kit=NX_KITS&&NX_KITS.find(function(k){return it.id==='garden-'+k.id;});if(kit&&!it.buffer&&kit.id!=='piano'){VOICES[kit.voice](t,kit.f,vel,voiceOut(it.vol,it.wet));return;}nxPlayInst(it,t,deg,vel);};
var nxInstPos=instPos;instPos=function(i){if(i<8)return nxInstPos(i);return {x:TWd*(.12+((i-8)%6)*.15),y:THd*.9};};
var nxDrawInstrument=drawInstrument;drawInstrument=function(i,t){if(i<8)nxDrawInstrument(i,t);};
VOICES.drumkit=function(t,f,v,d){VOICES.sub(t,58,v*.8,d);noise(t+.2,.14,d,2100,.8,'highpass',v*.14);VOICES.hat(t+.1,0,v*.7,d);VOICES.hat(t+.3,0,v*.6,d);};
var NX_KITS=[{id:'frame',name:'Rahmentrommel',voice:'frame',f:90,ico:'◯'},{id:'bongo',name:'Bongos',voice:'bongo',f:230,ico:'◉'},{id:'asia',name:'Gong & Schale',voice:'bowl',f:196,ico:'◎'},{id:'djembe',name:'Djembe',voice:'djembe',f:125,ico:'◈'},{id:'kit',name:'Schlagzeug',voice:'drumkit',f:58,ico:'●'},{id:'piano',name:'Klavier',voice:'piano',f:293.66,ico:'♫'}];
VOICES.bongo=function(t,f,v,d){VOICES.frame(t,f||230,v*.65,d);noise(t,.06,d,1800,2,'bandpass',v*.1);};
VOICES.djembe=function(t,f,v,d){VOICES.frame(t,f||125,v*.8,d);noise(t,.12,d,950,2,'bandpass',v*.18);};
VOICES.piano=function(t,f,v,d){var c=ctx();[1,2,3,4,6].forEach(function(h,i){var o=c.createOscillator(),g=c.createGain();o.frequency.value=f*h;o.type='sine';env(g,t,.004,.2*v/[1,3,6,9,15][i],.35/(i+1),.12);o.connect(g);g.connect(d);o.start(t);o.stop(t+2.8);});};
function nxTrack(id,name,voice){var it=INSTR.find(function(x){return x.id===id;});if(it)return it;it={id:id,name:name,ico:'♪',voice:voice,oct:1,wet:.25,pat:[0,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null],on:false,vol:.6,sample:null,buffer:null};INSTR.push(it);return it;}
NX_KITS.forEach(function(k){var it=nxTrack('garden-'+k.id,k.name,k.voice);it.pat=k.id==='piano'?[0,null,null,null,2,null,null,null,4,null,null,null,2,null,null,null]:k.id==='asia'?[0,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null]:[0,null,null,null,0,null,null,null,0,null,null,null,0,null,null,null];});
function nxInstrument(k){var it=nxTrack('garden-'+k.id,k.name,k.voice);playDrum({voice:k.voice,f:k.f,vol:.65},ctx().currentTime+.01,.8);ST.sel=it.id;toast(k.name+' · im Studio verfügbar');}
// Distinct generated animal voices; users can replace each with a real recording.
function nxAnimalVoice(name,t,f,v,d){var bird=/Adler|Eule|Fuchs|Kolibri|Rabe|Vogel/.test(name),owl=name==='Eule';var c=ctx(),dur=bird?(owl?.6:.25):1.1;var o=c.createOscillator(),g=c.createGain();o.type=bird?'sine':'triangle';var start=owl?390:bird?1700:160;o.frequency.setValueAtTime(start,t);o.frequency.exponentialRampToValueAtTime(owl?270:bird?2800:340,t+dur*.35);o.frequency.exponentialRampToValueAtTime(owl?230:bird?1300:190,t+dur);env(g,t,.06,.12*v,.15,dur);o.connect(g);g.connect(d);o.start(t);o.stop(t+dur+1);if(!bird)noise(t,dur,d,420,1,'bandpass',v*.025);}
function nxAnimalTrack(name){var id='animal-'+ANIMALS.findIndex(function(a){return a.n===name;});VOICES[id]=function(t,f,v,d){nxAnimalVoice(name,t,f,v,d);};return nxTrack(id,name, id);}
NX.discovered.forEach(nxAnimalTrack);
function nxDiscover(name){if(!NX.discovered.includes(name)){NX.discovered.push(name);store('vr_discovered',JSON.stringify(NX.discovered));toast(name+' entdeckt · neue Studio-Spur');}var it=nxAnimalTrack(name);playInst(it,ctx().currentTime+.01,0,.8);studioPaint();}
var nxAtlasAnimals=['Wolf','Eule','Hirsch','Fuchs','Adler','Tiger'],nxAnimalAtlas=null;
if(!PIXEL){nxAnimalAtlas=new Image();nxAnimalAtlas.src=nxAsset('animals-atlas.png');}
var nxSpiritT=0,nxSpirit=drawSpirit;drawSpirit=function(t,d,h){nxSpiritT=t;if(NX.animals)nxSpirit(t,d,h);};
var animalTarget=document.createElement('button');animalTarget.id='nx-animal-target';animalTarget.hidden=true;($('vr-ui')||bg).appendChild(animalTarget);
animalTarget.onclick=function(){if(spirit)nxDiscover(spirit.A.n);};
afterFrame.push(function(){if(!spirit||!NX.animals||nxSpiritT-spirit.t0<2.4||nxSpiritT-spirit.t0>7.5||tSec.getBoundingClientRect().top<H*.8){animalTarget.hidden=true;return;}animalTarget.hidden=false;animalTarget.textContent='♫ '+spirit.A.n+' entdecken';animalTarget.style.left=clamp(spirit.x-spirit.sz*.48,8,W-spirit.sz)+'px';animalTarget.style.top=(spirit.y-spirit.sz*.5)+'px';animalTarget.style.width=spirit.sz+'px';animalTarget.style.height=spirit.sz+'px';});
// Continuous, smoothly faded environmental sources. All start only after interaction.
function nxAmbience(name,value){value=clamp(Number(value)||0,0,1);NX.ambience[name]=value;var c=ctx(),n=NX.ambNodes[name];if(!n&&value>0){var source=c.createBufferSource(),filter=c.createBiquadFilter(),gain=c.createGain();source.buffer=noiseBuf(5);source.loop=true;filter.type=name==='rain'?'highpass':'lowpass';filter.frequency.value={fire:420,waterfall:3400,river:1250,rain:2100,wind:500}[name];gain.gain.value=0;source.connect(filter);filter.connect(gain);gain.connect(studioBus());source.start();n=NX.ambNodes[name]={source:source,gain:gain};}if(n)n.gain.gain.setTargetAtTime(value*{fire:.09,waterfall:.11,river:.075,rain:.045,wind:.055}[name],c.currentTime,.18);if(name==='fire')WS.fire=value>0;studio.dataset.atmosphere=JSON.stringify(NX.ambience);}
var nxAmbTick=0;afterFrame.push(function(t){if(t-nxAmbTick>.7){nxAmbTick=t;if(NX.ambience.fire>0&&actx)VOICES.crackle(actx.currentTime,0,NX.ambience.fire,studioBus());}if(NX.weather==='rain')WS.rainT=tClock;});
function nxOpenAmbience(name){ST.open=true;NX.tab='visuals';window.vrOpenStudio();if(!NX.ambience[name])nxAmbience(name,.25);studioPaint();var el=studio.querySelector('[data-nx-range="amb-'+name+'"]');if(el)el.focus();}
// Use existing hit-tested campfire instead of a second disconnected control.
tc.addEventListener('pointerdown',function(e){var p=tPos(e),h=hitAt(p[0],p[1]);if(h&&h.k==='fire'){e.stopImmediatePropagation();nxOpenAmbience('fire');}},{capture:true});
var nxFire=document.querySelector('[data-fire]');if(nxFire)nxFire.onclick=function(){nxOpenAmbience('fire');};
// Visible flowing water is drawn over the left edge of the photographic garden.
var nxLake=drawLake;drawLake=function(t){nxLake(t);tg.save();tg.globalCompositeOperation='screen';var x=TWd*.105,y=THd*.24,h=THd*.42;for(var i=0;i<26;i++){var xx=x+Math.sin(i*5.3)*TWd*.023,off=(t*100+i*27)%h;tg.strokeStyle='rgba(190,226,239,'+(.035+(i%5)*.015)+')';tg.lineWidth=1+i%3;tg.beginPath();tg.moveTo(xx,y+off);tg.bezierCurveTo(xx-7,y+off+16,xx+9,y+off+32,xx,y+Math.min(h,off+60));tg.stroke();}tg.restore();hit('waterfall',x,y+h/2,TWd*.05);hit('river',TWd*.6,THd*.6,TWd*.08);};
tc.addEventListener('pointerdown',function(e){var p=tPos(e),h=hitAt(p[0],p[1]);if(h&&(h.k==='waterfall'||h.k==='river')){e.stopImmediatePropagation();nxOpenAmbience(h.k);}},{capture:true});
var NX_FX=['Reverb','Delay','Chorus','Flanger','Phaser','Tremolo','Saturation','Low-pass','High-pass','Compressor'];
function nxAudioGraph(){if(NX.nodes)return NX.nodes;var c=ctx(),out=studioBus();reverbSend.gain.value=0;out.disconnect();var input=c.createGain(),low=c.createBiquadFilter(),high=c.createBiquadFilter(),sat=c.createWaveShaper(),comp=c.createDynamicsCompressor(),trem=c.createGain(),an=c.createAnalyser();low.type='lowpass';low.frequency.value=20000;high.type='highpass';high.frequency.value=20;comp.threshold.value=0;comp.ratio.value=1;sat.curve=null;input.connect(low);low.connect(high);high.connect(sat);sat.connect(comp);comp.connect(trem);trem.connect(master);trem.connect(an);an.fftSize=1024;out.connect(input);var delay=c.createDelay(2),feedback=c.createGain(),wet=c.createGain();delay.delayTime.value=.375;feedback.gain.value=.28;wet.gain.value=0;input.connect(delay);delay.connect(feedback);feedback.connect(delay);delay.connect(wet);wet.connect(low);
var mods={};['Chorus','Flanger','Phaser'].forEach(function(name){var mod=name==='Phaser'?c.createBiquadFilter():c.createDelay(.08),lfo=c.createOscillator(),depth=c.createGain(),gain=c.createGain();if(name==='Phaser'){mod.type='allpass';mod.frequency.value=600;depth.gain.value=400;lfo.frequency.value=.3;lfo.connect(depth);depth.connect(mod.frequency);}else{mod.delayTime.value=name==='Chorus'?.025:.006;depth.gain.value=name==='Chorus'?.008:.004;lfo.frequency.value=name==='Chorus'?.8:.18;lfo.connect(depth);depth.connect(mod.delayTime);}gain.gain.value=0;input.connect(mod);mod.connect(gain);gain.connect(low);lfo.start();mods[name]=gain;});var tlfo=c.createOscillator(),td=c.createGain();tlfo.frequency.value=4;td.gain.value=0;tlfo.connect(td);td.connect(trem.gain);tlfo.start();NX.nodes={input:input,low:low,high:high,sat:sat,comp:comp,trem:trem,an:an,delay:delay,wet:wet,mods:mods,td:td};return NX.nodes;}
function nxFx(name){var n=nxAudioGraph(),on=NX.fx[name]=!NX.fx[name],c=ctx();if(name==='Reverb')reverbSend.gain.setTargetAtTime(on?.48:0,c.currentTime,.05);if(name==='Delay')n.wet.gain.setTargetAtTime(on?.24:0,c.currentTime,.05);if(n.mods[name])n.mods[name].gain.setTargetAtTime(on?.3:0,c.currentTime,.05);if(name==='Tremolo'){n.td.gain.setTargetAtTime(on?.25:0,c.currentTime,.05);n.trem.gain.setTargetAtTime(on?.75:1,c.currentTime,.05);}if(name==='Low-pass')n.low.frequency.setTargetAtTime(on?2400:20000,c.currentTime,.05);if(name==='High-pass')n.high.frequency.setTargetAtTime(on?320:20,c.currentTime,.05);if(name==='Compressor'){n.comp.threshold.value=on?-18:0;n.comp.ratio.value=on?3:1;}if(name==='Saturation'){var curve=new Float32Array(1024);for(var i=0;i<1024;i++){var x=i/511.5-1;curve[i]=Math.tanh(x*2)/Math.tanh(2);}n.sat.curve=on?curve:null;}studioPaint();}
// Master EQ and dynamics affect room-generated audio; embedded players retain their own processing.
var nxMaster=null;function nxMasterGraph(){if(nxMaster)return nxMaster;var c=ctx(),lo=c.createBiquadFilter(),hi=c.createBiquadFilter(),lim=c.createDynamicsCompressor(),gain=c.createGain();lo.type='lowshelf';lo.frequency.value=160;hi.type='highshelf';hi.frequency.value=5000;lim.threshold.value=-3;lim.knee.value=4;lim.ratio.value=12;lim.attack.value=.003;lim.release.value=.2;gain.gain.value=.85;master.disconnect();master.connect(lo);lo.connect(hi);hi.connect(lim);lim.connect(gain);gain.connect(c.destination);gain.connect(vizAn);return nxMaster={lo:lo,hi:hi,lim:lim,gain:gain};}
var nxMasterValues={warmth:0,air:0,output:.85};
function nxApplyMain(v){VOL.main=v;store('vr_vmain',String(v));pl.querySelectorAll('audio,video').forEach(function(a){a.volume=v;});applyVolume('main');}
function nxMainControllable(){return !!pl.querySelector('audio,video')||embedKind==='youtube'||embedKind==='soundcloud';}
// Preserve original sequence/sample/MIDI implementation; add explicit labels and tabs.
var nxOldStudioHtml=studioHtml;
studioHtml=function(){var h='<header class="nx-studio-head"><div><small>YOU ARE NEO / SOUND SANCTUARY</small><h3>Atmosphere Studio</h3></div><button data-nx="close" aria-label="Studio schließen">✕</button></header><div class="nx-tabs" role="tablist" aria-label="Studio-Bereiche"><button role="tab" data-nx="music" aria-selected="'+(NX.tab==='music')+'">Musik</button><button role="tab" data-nx="visuals" aria-selected="'+(NX.tab==='visuals')+'">Visuals & Atmosphäre</button></div>';
if(NX.tab==='visuals')return h+'<div class="nx-body" role="tabpanel"><h4>Deine Lichtstimmung</h4>'+nxRange('light','Licht',NX.light,0,1,.01)+nxRange('plants','Pflanzen',NX.plants,0,1,.01)+nxRange('visuals','Mandalas & Energie',MIX.vis,0,1,.01)+'<div class="nx-buttons">'+nxButton('animals','Tiere '+(NX.animals?'an':'aus'),NX.animals)+nxButton('motion',visualsPaused?'Bewegung fortsetzen':'Bewegung pausieren',visualsPaused)+'</div><h4>Wetter</h4><div class="nx-buttons">'+['clear','rain','snow','mist'].map(function(w,i){return nxButton('weather-'+w,['Klar','Regen','Schnee','Nebel'][i],NX.weather===w);}).join('')+'</div><h4>Umgebungsklänge</h4><p class="nx-note">Sanft erzeugte Klangtexturen · unabhängig voneinander mischbar.</p>'+Object.keys(NX.ambience).map(function(k){return nxRange('amb-'+k,{fire:'Lagerfeuer',waterfall:'Wasserfall',river:'Fluss / Gletscherbach',rain:'Regen',wind:'Wind'}[k],NX.ambience[k],0,1,.01);}).join('')+'<div class="nx-buttons">'+nxButton('amb-off','Alle Umgebungsklänge aus',false)+nxButton('quiet','Ruhe-Preset',false)+nxButton('forest','Waldregen-Preset',false)+'</div><h4>Mondansicht</h4><label>Hemisphäre<select aria-label="Hemisphäre" data-nx-select="hemisphere"><option value="north">Nordhalbkugel</option><option value="south">Südhalbkugel</option><option value="equator">Äquator</option></select></label><p class="nx-note">Mondphase nach Datum; Blickrichtung als gestalterische Ansicht. Einstellung auf diesem Gerät.</p></div>';
h+='<div class="nx-body" role="tabpanel"><div class="nx-sync"><h4>Tempo & Synchronisation</h4><div class="nx-buttons">'+nxButton('sync-main','Playertempo → Studio',TEMPO.chief==='main')+nxButton('sync-studio','Studio → Visuals',TEMPO.chief==='studio')+nxButton('sync-free','Freies Tempo',TEMPO.chief==='hand')+'</div><p class="nx-note">Player-BPM tippen oder eintippen. Die Wiedergabegeschwindigkeit externer Player bleibt beim Anbieter.</p></div><div class="nx-buttons">'+nxButton('chimes','Glockenspiel '+(NX.chimes?'AN · ausschalten':'AUS · einschalten'),NX.chimes)+nxButton('all-stop','Alles im Studio stoppen',false)+'</div><h4>Effekte · Studio-Bus</h4><div class="nx-fx">'+NX_FX.map(function(n){return nxButton('fx-'+n,n,!!NX.fx[n]);}).join('')+'</div><div class="nx-buttons">'+nxButton('book','Instrumente',false)+'</div><div class="nx-legacy">'+nxOldStudioHtml()+'</div></div>';return h;};
var nxPaint=window.vrStudioPaint;window.vrStudioPaint=function(){nxPaint();if(!ST.open)return;studio.querySelectorAll('[title]').forEach(function(b){if(b.tagName==='BUTTON'){b.setAttribute('aria-label',b.title);if(b.dataset.ion||b.dataset.drumon){var item=(b.dataset.ion?INSTR:DRUMS).find(function(i){return i.id===(b.dataset.ion||b.dataset.drumon);});if(item){b.textContent=item.ico||'♪';b.title=item.name+' · Schleife an/aus';}}else if(b.dataset.s&&['play','stop','restart','tap','save','load','upload','lib'].includes(b.dataset.s)){b.textContent={play:SEQ.on?'Ⅱ':'▶',stop:'■',restart:'↻',tap:'☝',save:'▣',load:'▤',upload:'↑',lib:'▥'}[b.dataset.s];}}});studio.querySelectorAll('[data-cell]').forEach(function(cell){cell.tabIndex=0;cell.setAttribute('role','button');var p=cell.dataset.cell.split(':');cell.setAttribute('aria-label',p[0]+' Schritt '+(+p[1]+1));cell.setAttribute('aria-pressed',cell.classList.contains('on'));});var hs=studio.querySelector('[data-nx-select="hemisphere"]');if(hs)hs.value=window.VR_HEMISPHERE||'north';};
// Keep studio on the sticky interface layer, above the scene and outside transform effects.
($('vr-ui')||bg).appendChild(studio);
studio.addEventListener('keydown',function(e){if(e.target.dataset.cell&&(e.key==='Enter'||e.key===' ')){e.preventDefault();e.target.click();e.target.setAttribute('aria-pressed',e.target.classList.contains('on'));}});
studio.addEventListener('click',function(e){var b=e.target.closest('[data-nx]');if(!b)return;var a=b.dataset.nx;ctx();if(a==='music'||a==='visuals'){NX.tab=a;studioPaint();studio.scrollTop=0;}if(a==='close')closeStudio();if(a==='chimes'){NX.chimes=!NX.chimes;store('vr_chimes',NX.chimes?'on':'off');if(!NX.chimes)INSTR.find(function(i){return i.id==='chimes';}).on=false;studioPaint();}if(a==='all-stop'){window.vrStudio.stop();Object.keys(NX.ambience).forEach(function(k){nxAmbience(k,0);});NX.chimes=false;store('vr_chimes','off');studioPaint();}if(a.startsWith('fx-'))nxFx(a.slice(3));if(a.startsWith('add-')){var k=NX_KITS.find(function(k){return k.id===a.slice(4);});var it=nxTrack('garden-'+k.id,k.name,k.voice);it.on=!it.on;ensureClock();studioPaint();}if(a.startsWith('animal-'))nxDiscover(ANIMALS[+a.slice(7)].n);if(a.startsWith('sync-')){TEMPO.chief={main:'main',studio:'studio',free:'hand'}[a.slice(5)];setSync(a!=='sync-free');store('vr_chief',TEMPO.chief);studioPaint();}if(a.startsWith('weather-')){NX.weather=a.slice(8);studioPaint();}if(a==='animals'){NX.animals=!NX.animals;studioPaint();}if(a==='motion'){$('vr-motion').click();studioPaint();}if(a==='amb-off'||a==='quiet'||a==='forest'){Object.keys(NX.ambience).forEach(function(k){nxAmbience(k,a==='forest'?({rain:.3,river:.15,wind:.1}[k]||0):0);});if(a==='quiet'){NX.chimes=false;store('vr_chimes','off');NX.weather='clear';}if(a==='forest')NX.weather='rain';studioPaint();}});
studio.addEventListener('input',function(e){var k=e.target.dataset.nxRange;if(!k)return;var v=+e.target.value,output=e.target.parentElement.querySelector('output');if(output)output.textContent=v;if(k.startsWith('amb-'))nxAmbience(k.slice(4),v);if(k==='light'){NX.light=v;root.style.setProperty('--nx-light',v);}if(k==='plants')NX.plants=v;if(k==='visuals')setVis(v);});
studio.addEventListener('change',function(e){if(e.target.dataset.nxSelect==='hemisphere'&&window.VR_SET_HEMISPHERE)window.VR_SET_HEMISPHERE(e.target.value);});
var nxFerns=drawFerns;drawFerns=function(t,g,s){gf.save();gf.globalAlpha=NX.plants;nxFerns(t,g,s);gf.restore();};
var nxAtmosCanvas=document.createElement('canvas');nxAtmosCanvas.id='nx-weather';bg.appendChild(nxAtmosCanvas);
afterFrame.push(function(t){var c=nxAtmosCanvas,g=c.getContext('2d');if(c.width!==Math.round(W)||c.height!==Math.round(H)){c.width=Math.round(W);c.height=Math.round(H);}g.clearRect(0,0,W,H);g.fillStyle='rgba('+(NX.light<.5?'3,7,20':'242,189,112')+','+(Math.abs(NX.light-.5)*.16)+')';g.fillRect(0,0,W,H);if(NX.weather==='mist'){g.fillStyle='rgba(175,195,208,.10)';g.fillRect(0,0,W,H);}if(NX.weather==='rain'||NX.weather==='snow'){g.strokeStyle='rgba(190,217,233,.35)';g.fillStyle='rgba(240,247,251,.65)';for(var i=0;i<(PIXEL?35:90);i++){var x=(i*137.5+t*(NX.weather==='snow'?8:22))%W,y=(i*91+t*(NX.weather==='snow'?20:280))%H;if(NX.weather==='snow'){g.beginPath();g.arc(x,y,1+i%3*.4,0,7);g.fill();}else{g.beginPath();g.moveTo(x,y);g.lineTo(x-3,y+15);g.stroke();}}}});
// Persistent right-side monitor, separate real signals in cyan and amber.
var monitor=document.createElement('section');monitor.id='nx-monitor';monitor.innerHTML='<header><small>LIVE AUDIO</small><strong>Wave Observatory</strong></header><div class="nx-buttons"><button data-source="studio">Studio</button><button data-source="player">Player</button><button data-source="all" class="on">Beide</button></div><canvas width="512" height="170" aria-label="Live Audio Analyzer"></canvas><div class="nx-buttons"><button data-view="spectrum" class="on">Spektrum</button><button data-view="wave">Welle</button><button data-view="level">Pegel</button></div><p id="nx-signal-status" class="nx-note">Studio: bereit · Player: noch kein Analysesignal</p><button data-monitor="capture">Player-Signal verbinden</button><h4>Mix & Master</h4>'+nxRange('studio','Studio',VOL.studio,0,1,.01)+nxRange('main','Player',VOL.main,0,1,.01)+'<p id="nx-player-note" class="nx-note"></p>'+nxRange('balance','Studio ↔ Player',.5,0,1,.01)+'<details><summary>Master · Raumklänge</summary>'+nxRange('warmth','Wärme',0,-6,6,.5)+nxRange('air','Brillanz',0,-6,6,.5)+nxRange('output','Ausgang',.85,0,1,.01)+'<button data-monitor="master">Auto-Master · sanft</button><button data-monitor="ai">AI-Mastering</button><p class="nx-note" id="nx-master-status">Lokaler EQ & Limiter. Externe Player bleiben separat.</p></details>';
$('vr-aside').prepend(monitor);
monitor.addEventListener('input',function(e){var k=e.target.dataset.nxRange;if(!k)return;var v=+e.target.value;var o=e.target.parentElement.querySelector('output');if(o)o.textContent=v;if(k==='studio'){VOL.studio=v;studioBus();window.vrStudio.volume(v);}if(k==='main')nxApplyMain(v);if(k==='balance'){VOL.studio=1-v;studioBus();window.vrStudio.volume(1-v);nxApplyMain(v);monitor.querySelector('[data-nx-range="studio"]').value=1-v;monitor.querySelector('[data-nx-range="main"]').value=v;}if(['warmth','air','output'].includes(k)){var m=nxMasterGraph();nxMasterValues[k]=v;({warmth:m.lo.gain,air:m.hi.gain,output:m.gain.gain}[k]).setTargetAtTime(v,ctx().currentTime,.05);}});
monitor.onclick=function(e){var b=e.target.closest('button');if(!b)return;if(b.dataset.source){NX.analyserSource=b.dataset.source;monitor.querySelectorAll('[data-source]').forEach(function(x){x.classList.toggle('on',x===b);});}if(b.dataset.view){NX.analyserMode=b.dataset.view;monitor.querySelectorAll('[data-view]').forEach(function(x){x.classList.toggle('on',x===b);});}if(b.dataset.monitor==='capture'){$('nx-signal-status').textContent='Tab-Audio teilt den gesamten Tab-Mix. Für einen isolierten Player nutze Eigene Musik.';shareTabAudio();}if(b.dataset.monitor==='master'){var m=nxMasterGraph();m.lo.gain.value=1;m.hi.gain.value=.5;m.lim.threshold.value=-8;m.lim.ratio.value=4;$('nx-master-status').textContent='Sanftes Master-Preset aktiv · EQ, Kompression & Ausgangsschutz.';}if(b.dataset.monitor==='ai'){$('nx-master-status').textContent='Noch kein AI-Mastering-Dienst verbunden. Das lokale Auto-Master-Preset funktioniert ohne Upload.';}};
var nxMonitorT=0;function nxDrawMonitor(t){if(t-nxMonitorT<.05)return;nxMonitorT=t;var c=monitor.querySelector('canvas'),g=c.getContext('2d'),w=c.width,h=c.height;g.fillStyle='#081119';g.fillRect(0,0,w,h);g.strokeStyle='#203039';for(var i=1;i<4;i++){g.beginPath();g.moveTo(0,i*h/4);g.lineTo(w,i*h/4);g.stroke();}var playerAn=nxPlayerAn||(tabStream?analyser:null);var items=[{an:NX.nodes?NX.nodes.an:vizAn,color:'#73ded5',key:'studio'},{an:playerAn,color:'#e7b676',key:'player'}];items.forEach(function(it,j){if(!it.an||(NX.analyserSource!=='all'&&NX.analyserSource!==it.key))return;var data=new Uint8Array(it.an.frequencyBinCount);if(NX.analyserMode==='wave')it.an.getByteTimeDomainData(data);else it.an.getByteFrequencyData(data);g.strokeStyle=it.color;g.fillStyle=it.color;g.lineWidth=2;g.beginPath();if(NX.analyserMode==='wave'){for(var k=0;k<data.length;k++){var x=k/data.length*w,y=data[k]/255*h;k?g.lineTo(x,y):g.moveTo(x,y);}g.stroke();}else if(NX.analyserMode==='level'){var sum=0;data.forEach(function(v){sum+=v*v;});var level=Math.sqrt(sum/data.length)/255;g.fillRect(15,35+j*60,level*(w-30),28);}else{for(var k=0;k<48;k++){var idx=Math.floor(Math.pow(k/48,1.6)*(data.length*.8)),v=data[idx]/255;g.globalAlpha=j?.65:.8;g.fillRect(k*w/48+2+j*3,h-v*(h-10),5,v*(h-10));}g.globalAlpha=1;}});$('nx-signal-status').textContent='Studio: '+(actx?'aktiv':'bereit')+' · Player: '+(nxPlayerAn?'eigene Musik':playerAn?'geteiltes Tab-Signal':'kein Analysesignal');var capable=nxMainControllable();monitor.querySelector('[data-nx-range="main"]').disabled=!capable;monitor.querySelector('[data-nx-range="balance"]').disabled=!capable;$('nx-player-note').textContent=capable?'Player und Studio separat mischbar.':'Dieser Player regelt seine Lautstärke direkt im eingebetteten Player.';}
setInterval(function(){if(!document.hidden)nxDrawMonitor(performance.now()/1000);},50);
// Initialize the effects routing on first user gesture without autoplay.
root.addEventListener('pointerdown',function(){nxAudioGraph();nxMasterGraph();},{once:true});
syncBtn.innerHTML='SYNC';syncBtn.setAttribute('aria-label','Tempo und Synchronisation öffnen');
// Explicit playback controls are separate from the analyzer's source selector.
var nxTransport=document.createElement('div');nxTransport.className='nx-buttons';nxTransport.innerHTML='<button data-playback="studio">Studio starten</button><button data-playback="player">Player starten</button>';
monitor.querySelector('canvas').insertAdjacentElement('beforebegin',nxTransport);
nxTransport.onclick=function(e){var b=e.target.closest('button');if(!b)return;ctx();if(b.dataset.playback==='studio'){if(anyPlaying())window.vrStudio.stop();else window.vrStudio.play();}else{var media=pl.querySelector('audio,video');if(media){if(media.paused)media.play().catch(function(){toast('Bitte den Play-Knopf direkt im Player verwenden.');});else media.pause();}else toast('Bitte Start/Pause direkt im eingebetteten Player verwenden.');}};
afterFrame.push(function(){nxTransport.querySelector('[data-playback="studio"]').textContent=anyPlaying()?'Studio pausieren':'Studio starten';var media=pl.querySelector('audio,video');nxTransport.querySelector('[data-playback="player"]').textContent=media&&!media.paused?'Player pausieren':'Player starten';});
// Local audio is optional and stays on the device. It provides an isolated Player
// signal even when a third-party embedded player cannot expose its audio stream.
var nxLocal=document.createElement('details');nxLocal.innerHTML='<summary>Eigene Musik im Mixer</summary><label class="nx-note">Audiodatei von deinem Gerät<input type="file" accept="audio/*" aria-label="Eigene Musik für den Mixer"></label><audio controls style="width:100%"></audio><p class="nx-note">Bleibt auf deinem Gerät · eigener Player-Kanal und echte Wellenform.</p>';
monitor.appendChild(nxLocal);
var nxLocalAudio=nxLocal.querySelector('audio'),nxLocalURL=null,nxLocalGain=null,nxPlayerAn=null;
nxLocal.querySelector('input').onchange=function(e){var file=e.target.files[0];if(!file)return;var c=ctx();nxLocalAudio.pause();if(nxLocalURL)URL.revokeObjectURL(nxLocalURL);nxLocalURL=URL.createObjectURL(file);nxLocalAudio.src=nxLocalURL;if(!nxPlayerAn){var source=c.createMediaElementSource(nxLocalAudio);nxLocalGain=c.createGain();nxPlayerAn=c.createAnalyser();nxPlayerAn.fftSize=1024;source.connect(nxLocalGain);nxLocalGain.connect(nxPlayerAn);nxLocalGain.connect(c.destination);}nxLocalGain.gain.value=VOL.main;nxLocalAudio.play().catch(function(){toast('Deine Datei ist bereit. Drücke Play.');});};
var nxBaseMain=nxApplyMain;nxApplyMain=function(v){nxBaseMain(v);if(nxLocalGain)nxLocalGain.gain.setTargetAtTime(v,ctx().currentTime,.05);};
var nxBaseControllable=nxMainControllable;nxMainControllable=function(){return !!nxLocalURL||nxBaseControllable();};
// Button semantics and keyboard access for the two studio panels.
studio.addEventListener('keydown',function(e){if(e.target.getAttribute('role')==='tab'&&(e.key==='ArrowLeft'||e.key==='ArrowRight')){e.preventDefault();NX.tab=NX.tab==='music'?'visuals':'music';studioPaint();studio.querySelector('[data-nx="'+NX.tab+'"]').focus();}});


$('vr-aside-toggle').textContent='Mixer & Sammlung';
/* Final release: sound book, extended patterns and quiet view. */
NX.book=false;NX.bookPage='instruments';
var nxInstrumentAtlas=null;if(!PIXEL){nxInstrumentAtlas=new Image();nxInstrumentAtlas.src=nxAsset('instruments-atlas.png');}
var nxFloorInstrument=drawInstrument;
drawInstrument=function(i,t){
 if(PIXEL||!nxInstrumentAtlas||!nxInstrumentAtlas.complete||!nxInstrumentAtlas.naturalWidth){nxFloorInstrument(i,t);return;}
 var it=INSTR[i],k=NX_KITS.findIndex(function(k){return it.id==='garden-'+k.id;});
 if(k<0)return;
 var w=Math.min(TWd*.145,155),h=w*.9,x=TWd*(.09+k*.164),y=THd*.84+Math.sin(k*2)*THd*.045;
 tg.save();tg.globalCompositeOperation='screen';var cw=nxInstrumentAtlas.naturalWidth/3,ch=nxInstrumentAtlas.naturalHeight/2;
 tg.drawImage(nxInstrumentAtlas,(k%3)*cw,Math.floor(k/3)*ch,cw,ch,x-w/2,y-h/2,w,h);tg.restore();
 tg.fillStyle=it.on?'#c8ffbc':'#d8c294';tg.font='11px system-ui';tg.textAlign='center';tg.fillText(NX_KITS[k].name,x,y+h*.48);hit('instplay',x,y,w*.45,i);
};
function nxBookHTML(){var h='<section class="nx-book" aria-label="Klangbuch"><header><span>✧ Das Klangbuch</span><button data-nx="book-close" title="Buch schließen">✕</button></header><nav>'+['instruments','animals','ambience'].map(function(p,i){return '<button data-book-page="'+p+'" class="'+(NX.bookPage===p?'on':'')+'">'+['Instrumente','Krafttiere','Atmosphäre'][i]+'</button>';}).join('')+'</nav><div class="nx-book-pages">';
 if(NX.bookPage==='instruments')h+=NX_KITS.map(function(k,i){return '<button data-book-kit="'+k.id+'" title="'+k.name+' anspielen und als Spur aktivieren"><span class="nx-book-image" style="--x:'+((i%3)*50)+'%;--y:'+(i<3?0:100)+'%">'+(PIXEL?k.ico:'')+'</span><b>'+k.name+'</b><small>＋ Spur · ♪ anspielen</small></button>';}).join('');
 if(NX.bookPage==='animals')h+=NX.discovered.length?NX.discovered.map(function(n){return '<button data-book-animal="'+esc(n)+'">✧<b>'+esc(n)+'</b><small>＋ Spur · ♪ anspielen</small></button>';}).join(''):'<p>Entdecke ein Krafttier in den Mandalas und berühre es. Seine Klangstimme bleibt dann in deinem Buch.</p>';
 if(NX.bookPage==='ambience')h+=Object.keys(NX.ambience).map(function(k){return '<button data-book-amb="'+k+'"><b>'+({fire:'🔥 Lagerfeuer',waterfall:'≋ Wasserfall',river:'≈ Gletscherbach',rain:'☂ Regen',wind:'〰 Wind',drops:'⋮ Tropfen'}[k])+'</b><small>Lautstärke einstellen</small></button>';}).join('');
 return h+'</div><footer>'+nxButton('sample-book','↑ Eigene Klänge hinzufügen',false)+'</footer></section>';}
var nxFinalHTML=studioHtml;
studioHtml=function(){var h=nxFinalHTML();var seq='<div class="nx-pattern-tools"><button data-pattern="less" title="Einen Takt entfernen">−</button><output id="nx-play-position">'+(SEQ.bars||1)+' Takt'+((SEQ.bars||1)>1?'e':'')+'</output><button data-pattern="more" title="Einen leeren Takt hinzufügen">＋</button><button data-pattern="double" title="Muster verdoppeln und kopieren">⧉ ×2</button><button data-pattern="copy" title="Ersten Takt in alle weiteren Takte kopieren">⧉ →</button><select data-pattern-preset aria-label="Beat-Preset"><option value="">Beat-Preset…</option value="heartbeat">Herzschlag</option><option value="journey">Schamanische Reise</option><option value="island">Inselrhythmus</option></select><button data-pattern="half" title="Tempo halbieren">½ ♪</button><button data-pattern="twice" title="Tempo verdoppeln">2× ♪</button><button data-pattern="sub" title="Zwischenschritte im Shaker ergänzen">⋮ ♪</button></div>';
 h=h.replace('<div class="nx-legacy">','<div class="nx-legacy">'+seq);
 if(NX.book)h+=nxBookHTML();return h;};
var nxFinalPaint=window.vrStudioPaint;window.vrStudioPaint=function(){nxFinalPaint();if(!ST.open)return;if(!PIXEL)studio.style.setProperty('--book-atlas','url("'+nxAsset('instruments-atlas.png')+'")');studio.querySelectorAll('[data-nx]').forEach(function(b){var k=b.dataset.nx;var icons={ 'sync-main':'🎧 → ♪','sync-studio':'♪ → ✺','sync-free':'↔','chimes':NX.chimes?'🔔':'🔕','all-stop':'■','animals':'✧','motion':visualsPaused?'▶':'Ⅱ','weather-clear':'☀','weather-rain':'☂','weather-snow':'❄','weather-mist':'≋','amb-off':'⏻','quiet':'☾','forest':'♧','book':'▤'};if(icons[k])b.textContent=icons[k];});};
function nxBars(n,copy){var old=SEQ.steps,grids={};DRUMS.forEach(function(d){grids[d.id]=SEQ.grid[d.id].slice();});SEQ.bars=clamp(n,1,8);resetGrid();if(copy)DRUMS.forEach(function(d){for(var i=0;i<SEQ.steps;i++)SEQ.grid[d.id][i]=!!grids[d.id][i%old];});SEQ.step%=SEQ.steps;studioPaint();}
studio.addEventListener('click',function(e){var b=e.target.closest('button');if(!b)return;var a=b.dataset.nx;if(a==='book'){NX.book=!NX.book;studioPaint();studio.scrollTop=0;}if(a==='book-close'){NX.book=false;studioPaint();}if(a==='sample-book')uploadSample();if(b.dataset.bookPage){NX.bookPage=b.dataset.bookPage;studioPaint();}if(b.dataset.bookKit){var k=NX_KITS.find(function(k){return k.id===b.dataset.bookKit;});nxInstrument(k);var it=nxTrack('garden-'+k.id,k.name,k.voice);it.on=true;ensureClock();studioPaint();}if(b.dataset.bookAnimal){nxDiscover(b.dataset.bookAnimal);nxAnimalTrack(b.dataset.bookAnimal).on=true;ensureClock();studioPaint();}if(b.dataset.bookAmb){NX.book=false;nxOpenAmbience(b.dataset.bookAmb);}var p=b.dataset.pattern;if(p==='more')nxBars((SEQ.bars||1)+1,false);if(p==='less')nxBars((SEQ.bars||1)-1,false);if(p==='double')nxBars((SEQ.bars||1)*2,true);if(p==='copy'){var n=stepsFor(SEQ.meter);DRUMS.forEach(function(d){for(var i=n;i<SEQ.steps;i++)SEQ.grid[d.id][i]=SEQ.grid[d.id][i%n];});studioPaint();}if(p==='half'||p==='twice'){setBpm(clamp(TEMPO.bpm*(p==='half'?.5:2),20,300));studioPaint();}if(p==='sub'){for(var i=2;i<SEQ.steps;i+=4)SEQ.grid.shaker[i]=!SEQ.grid.shaker[i];studioPaint();}});
studio.addEventListener('change',function(e){if(!e.target.hasAttribute('data-pattern-preset'))return;var name=e.target.value;if(!name)return;DRUMS.forEach(function(d){SEQ.grid[d.id]=Array(SEQ.steps).fill(false);});for(var i=0;i<SEQ.steps;i++){var j=i%16;SEQ.grid.low[i]=(name==='heartbeat'?[0,3,8,11]:name==='journey'?[0,4,8,12]:[0,6,10]).includes(j);SEQ.grid.high[i]=(name==='island'?[4,7,12,15]:[12]).includes(j);SEQ.grid.shaker[i]=name==='journey'?j%2===0:j%4===2;}SEQ.on=true;SEQ.drumsOn=true;ensureClock();studioPaint();});
onStep.push(function(step){var out=studio.querySelector('#nx-play-position');if(out)out.textContent='Takt '+(Math.floor(step/stepsFor(SEQ.meter))+1)+' / '+(SEQ.bars||1)+' · '+(step%stepsFor(SEQ.meter)+1);studio.querySelectorAll('[data-cell]').forEach(function(c){c.classList.toggle('nx-playhead',+c.dataset.cell.split(':')[1]===step);});if(ST.open)studio.style.setProperty('--nx-pulse',String(step%4===0?.42:.16));});
var nxColor=155;function nxReact(){if(PIXEL)return;nxColor=(nxColor+7)%360;studio.style.setProperty('--nx-hue',nxColor);studio.style.setProperty('--nx-bloom',String(3+Math.floor(Math.random()*8)));}studio.addEventListener('input',nxReact);studio.addEventListener('click',nxReact);
// Quiet mode restores immediately with pointer, touch, key, focus or scrolling.
var nxLastActive=performance.now(),nxQuietTimer=document.createElement('div');nxQuietTimer.id='nx-quiet-timer';nxQuietTimer.setAttribute('aria-hidden','true');root.appendChild(nxQuietTimer);
function nxWake(){nxLastActive=performance.now();root.classList.remove('nx-idle');}
['pointermove','pointerdown','keydown','wheel','touchstart','focusin'].forEach(function(n){document.addEventListener(n,nxWake,{passive:true});});window.addEventListener('scroll',nxWake,{passive:true});
setInterval(function(){root.classList.toggle('nx-idle',!ST.open&&(!root.classList.contains('vr-cinematic')||root.classList.contains('uf-aside-closed'))&&performance.now()-nxLastActive>5000);nxQuietTimer.textContent=running?tEl.textContent:'';},250);
if(!PIXEL)setTimeout(function(){root.classList.add('nx-intro-done');},6500);
// Growing woodland layer: reveal the mature tree without scaling its anatomy.
var nxTree=null;if(!PIXEL){nxTree=new Image();nxTree.src=nxAsset('ancient-tree.png');var nxTreeFerns=drawFerns;drawFerns=function(t,g,s){if(nxTree.complete&&nxTree.naturalWidth){var grow=smooth(clamp((svS-.55)/1.7,0,1))*NX.plants,hh=H*.98,ww=hh*nxTree.naturalWidth/nxTree.naturalHeight;gf.save();gf.translate(W-ww*.5,H);gf.scale(.72+.28*grow,.2+.8*grow);gf.translate(-(W-ww*.5),-H);gf.globalCompositeOperation='screen';gf.globalAlpha=.82*grow;gf.drawImage(nxTree,W-ww,H-hh,ww,hh);gf.restore();}nxTreeFerns(t,g,s);};}
NX.ambience.drops=0;
var nxAmbBase=nxAmbience;nxAmbience=function(name,value){if(name==='drops'){NX.ambience.drops=clamp(Number(value)||0,0,1);ctx();studio.dataset.atmosphere=JSON.stringify(NX.ambience);return;}nxAmbBase(name,value);};
var nxDropTime=0;afterFrame.push(function(t){if(NX.ambience.drops>0&&actx&&t-nxDropTime>1.3){nxDropTime=t;VOICES.bowl(actx.currentTime,900+Math.random()*700,NX.ambience.drops*.1,studioBus());}});
var nxAmbLabels=studioHtml;studioHtml=function(){return nxAmbLabels().replace('undefined <output>','Einzelne Tropfen <output>');};
var nxGatherFire=drawFire;drawFire=function(t){nxGatherFire(t);if(PIXEL||!NX.animals||!nxAnimalAtlas||!nxAnimalAtlas.complete||!nxAnimalAtlas.naturalWidth)return;NX.discovered.slice(0,6).forEach(function(n,i){var k=nxAtlasAnimals.indexOf(n);if(k<0)return;var w=Math.min(88,TWd*.095),x=TWd*(.2+i*.115),y=THd*.69,cw=nxAnimalAtlas.naturalWidth/3,ch=nxAnimalAtlas.naturalHeight/2;tg.save();tg.globalCompositeOperation='screen';tg.globalAlpha=.9;tg.drawImage(nxAnimalAtlas,(k%3)*cw,Math.floor(k/3)*ch,cw,ch,x-w/2,y-w/2,w,w);tg.restore();hit('nx-animal',x,y,w*.45,n);});};
tc.addEventListener('pointerdown',function(e){var p=tPos(e),h=hitAt(p[0],p[1]);if(h&&h.k==='nx-animal'){e.stopImmediatePropagation();nxDiscover(h.d);}},{capture:true});
NX.ambMemory={};var nxPowerRange=nxRange;nxRange=function(id,label,v,min,max,step){var html=nxPowerRange(id,label,v,min,max,step);if(id.startsWith('amb-'))html=html.replace('</label>','<button type="button" data-amb-toggle="'+id.slice(4)+'" title="'+label+' an/aus" aria-label="'+label+' an/aus" aria-pressed="'+(v>0)+'">⏻</button></label>');return html;};
var nxPowerHTML=studioHtml;studioHtml=function(){return nxPowerHTML().replace('<h4>Umgebungsklänge</h4>','<h4>Umgebungsklänge</h4>'+nxButton('amb-power','Alle Umgebungsklänge an/aus',Object.values(NX.ambience).some(function(v){return v>0;})));};
studio.addEventListener('click',function(e){var b=e.target.closest('button');if(!b)return;var key=b.dataset.ambToggle;if(key){if(NX.ambience[key]>0){NX.ambMemory[key]=NX.ambience[key];nxAmbience(key,0);}else nxAmbience(key,NX.ambMemory[key]||.2);studioPaint();}if(b.dataset.nx==='amb-power'){var on=Object.values(NX.ambience).some(function(v){return v>0;});Object.keys(NX.ambience).forEach(function(k){if(on){if(NX.ambience[k]>0)NX.ambMemory[k]=NX.ambience[k];nxAmbience(k,0);}else nxAmbience(k,NX.ambMemory[k]||.12);});studioPaint();}});
if(!PIXEL){
 var nxOldGong=drawGong;drawGong=function(t,dt){if(!nxInstrumentAtlas||!nxInstrumentAtlas.complete||!nxInstrumentAtlas.naturalWidth)return nxOldGong(t,dt);var cw=nxInstrumentAtlas.naturalWidth/3,ch=nxInstrumentAtlas.naturalHeight/2,w=Math.min(TWd*.23,220),h=w*ch/cw;tg.save();tg.globalCompositeOperation='screen';tg.drawImage(nxInstrumentAtlas,2*cw,0,cw,ch,G.gong.x-w/2,G.gong.y-h/2,w,h);tg.restore();};
 drawGong=nxOldGong;
}
/* Sanctuary: one instrument assignment shared by the book, sequencer and garden. */
var SAN={page:'music',slot:0,slots:Array.from({length:16},function(_,i){return ['frame','bongo','djembe','asia','piano','frame'][i%6];}),kit:'frame',dragonAt:-100};
try{var saved=JSON.parse(store('vr_sanctuary')||'null');if(saved&&Array.isArray(saved.slots)&&saved.slots.length===16){SAN.slots=saved.slots.map(function(k){return NX_KITS.some(function(x){return x.id===k;})?k:'frame';});SAN.kit=NX_KITS.some(function(k){return k.id===saved.kit;})?saved.kit:'frame';}}catch(e){}
function sanSave(){store('vr_sanctuary',JSON.stringify({slots:SAN.slots,kit:SAN.kit}));}
var SAN_COLORS=['#dda968','#72c9b1','#af96ee','#77bfee','#dc8f9c','#e5cc88'];
function sanPicture(type,color){var paths={fire:'M48 77C20 65 33 47 39 39Q38 53 46 54Q40 27 61 14Q55 37 69 46Q85 66 64 79Q71 58 56 49Q59 66 48 77Z',waterfall:'M24 18H72M32 18V56Q32 70 22 77M48 18V65M63 18V55Q63 73 77 79M18 84Q32 76 45 84T78 84',river:'M20 16Q85 32 42 49T67 84M36 13Q97 30 59 49T80 81',rain:'M24 46C8 44 13 25 29 27C32 8 63 9 67 27C88 24 92 47 73 47ZM30 58L24 72M50 58L44 72M70 58L64 72M39 79L35 88',wind:'M14 32H64Q89 32 80 18Q74 10 66 19M10 48H76Q96 48 86 60M19 65H55Q78 65 67 80Q61 85 56 78',drops:'M48 13Q17 51 27 68Q47 94 69 69Q82 53 48 13ZM36 56Q32 69 45 73',Reverb:'M21 77V36L48 17L77 36V77M32 69V42L48 31L66 42V69M42 65V48H55V65',Delay:'M16 61V37M30 71V27M46 65V33M62 59V39M78 53V45',Chorus:'M12 35Q30 12 48 35T84 35M12 51Q30 28 48 51T84 51M12 67Q30 44 48 67T84 67',Flanger:'M12 25Q48 85 84 25M12 71Q48 11 84 71',Phaser:'M12 49Q30 8 48 49T84 49M12 49Q30 90 48 49T84 49',Tremolo:'M12 48L22 30L32 68L42 18L52 78L62 30L72 60L84 48',Saturation:'M12 73L30 23H64L84 73M12 80H84','Low-pass':'M12 25H46Q60 25 64 49L78 78','High-pass':'M12 78L31 42Q36 25 49 25H84',Compressor:'M12 19L32 38M12 77L32 58M84 19L64 38M84 77L64 58M38 34V63M58 34V63'};return '<svg viewBox="0 0 96 96" aria-hidden="true" style="color:'+color+'"><circle cx="48" cy="48" r="43" fill="currentColor" opacity=".07"/><path d="'+(paths[type]||paths.Chorus)+'" fill="'+(type==='fire'?'currentColor':'none')+'" stroke="currentColor" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';}
function sanAtlas(k){var i=NX_KITS.findIndex(function(x){return x.id===k;});return '<span class="san-photo" style="background-position:'+((i%3)*50)+'% '+(i<3?0:100)+'%"></span>';}
function sanKit(k){SAN.kit=k;var def=NX_KITS.find(function(x){return x.id===k;});DRUMS.forEach(function(d,i){if(i<3){d.voice=def.voice;d.f=def.f*[.72,1,1.45][i];d.buffer=null;d.sample=null;}});sanSave();}
sanKit(SAN.kit);
var sanHTML=studioHtml;
studioHtml=function(){NX.tab=SAN.page==='visuals'?'visuals':'music';var base=sanHTML(),end=base.indexOf('</div>',base.indexOf('class="nx-tabs"'))+6;var head=base.slice(0,base.indexOf('<div class="nx-tabs"'))+'<nav class="nx-tabs" role="tablist" aria-label="Klangbuch Kapitel">'+['music','instruments','ambience','visuals'].map(function(p,i){return '<button role="tab" aria-selected="'+(SAN.page===p)+'" data-san-page="'+p+'">'+['Musik & Effekte','Instrumente','Ambiente','Licht & Natur'][i]+'</button>';}).join('')+'</nav>';
if(SAN.page==='music'||SAN.page==='visuals')return head+base.slice(end);
var body='<div class="nx-body san-book"><p class="san-eyebrow">DAS KLANGBUCH / '+(SAN.page==='ambience'?'NATUR':'INSTRUMENTE')+'</p><h2>'+(SAN.page==='ambience'?'Die Welt atmet mit.':'Dein Kreis. Dein Klang.')+'</h2>';
if(SAN.page==='ambience'){var labels={fire:'Lagerfeuer',waterfall:'Wasserfall',river:'Gletscherbach',rain:'Regen',wind:'Wind',drops:'Wassertropfen'};body+='<p>Berühre Feuer oder Wasser im Garten, um hier ihren Klang zu mischen.</p><div class="san-cards">'+Object.keys(NX.ambience).map(function(k,i){return '<article style="--san-color:'+SAN_COLORS[i]+'">'+sanPicture(k,SAN_COLORS[i])+'<h3>'+labels[k]+'</h3>'+nxRange('amb-'+k,'Lautstärke',NX.ambience[k],0,1,.01)+'</article>';}).join('')+'</div><p class="nx-note">Synthetische Naturklänge · eigene Aufnahmen lassen sich als Samples ergänzen.</p>'+nxButton('amb-off','Alle Naturklänge aus',false);}
else {body+='<p>16 Plätze im Garten. Wähle einen Platz und tausche sein Instrument. Das Trommelensemble steht rechts neben dem Gong.</p><div class="san-slots">'+SAN.slots.map(function(k,i){return '<button data-san-slot="'+i+'" aria-pressed="'+(SAN.slot===i)+'">'+String(i+1).padStart(2,'0')+'</button>';}).join('')+'</div><div class="san-cards">'+NX_KITS.map(function(k,i){return '<article style="--san-color:'+SAN_COLORS[i]+'">'+sanAtlas(k.id)+'<h3>'+k.name+'</h3><button data-san-assign="'+k.id+'">Auf Platz '+(SAN.slot+1)+'</button><button data-san-preview="'+k.id+'">Anspielen</button></article>';}).join('')+'</div><h3>Trommelensemble</h3><p class="nx-note">Bild und Klangmodell wechseln gemeinsam. Eigene Samples ersetzen das Klangmodell über den Sample-Upload der Spur.</p><div class="san-kits">'+[['frame','Schamanische Trommeln'],['kit','Schlagzeug'],['djembe','Westafrikanische Djembe'],['asia','Asiatische Gongs & Schalen'],['bongo','Bongos']].map(function(k){return '<button data-san-kit="'+k[0]+'" aria-pressed="'+(SAN.kit===k[0])+'">'+k[1]+'</button>';}).join('')+'</div>';}
return head+body+'</div>';};
var sanPaint=window.vrStudioPaint;window.vrStudioPaint=function(){sanPaint();if(!ST.open)return;studio.style.setProperty('--book-atlas','url("'+nxAsset('instruments-atlas.png')+'")');studio.querySelectorAll('.nx-fx [data-nx]').forEach(function(b,i){var name=b.dataset.nx.slice(3),color=SAN_COLORS[i%6];b.style.setProperty('--san-color',color);b.innerHTML=sanPicture(name,color)+'<span>'+name+'</span>';});};
studio.addEventListener('click',function(e){var b=e.target.closest('button');if(!b)return;if(b.dataset.sanPage){SAN.page=b.dataset.sanPage;NX.book=false;studioPaint();studio.scrollTop=0;}if(b.dataset.sanSlot!==undefined){SAN.slot=+b.dataset.sanSlot;studioPaint();}if(b.dataset.sanAssign){SAN.slots[SAN.slot]=b.dataset.sanAssign;sanSave();studioPaint();toast('Platz '+(SAN.slot+1)+' aktualisiert');}if(b.dataset.sanPreview)nxInstrument(NX_KITS.find(function(k){return k.id===b.dataset.sanPreview;}));if(b.dataset.sanKit){sanKit(b.dataset.sanKit);studioPaint();}if(b.dataset.nx==='book'){NX.book=false;SAN.page='instruments';studioPaint();}});
var sanAmbOpen=nxOpenAmbience;nxOpenAmbience=function(k){SAN.page='ambience';sanAmbOpen(k);};
studio.addEventListener('keydown',function(e){if(e.target.dataset.sanPage&&['ArrowLeft','ArrowRight'].includes(e.key)){e.stopImmediatePropagation();e.preventDefault();var pages=['music','instruments','ambience','visuals'];SAN.page=pages[(pages.indexOf(SAN.page)+(e.key==='ArrowRight'?1:3))%4];studioPaint();studio.querySelector('[data-san-page="'+SAN.page+'"]').focus();}},{capture:true});
// The clock and player share the actual side-column dock coordinates.
window.vrDockC=function(pw,ph,C){if(W<981||PMODE!=='dock'){adock.style.height='0px';return C;}var a=aside.getBoundingClientRect(),b=bg.getBoundingClientRect(),scale=Math.min((a.width-28)/pw,.62),height=ph*scale;adock.style.height=(height+122)+'px';return {px:a.left-b.left+14,py:a.bottom-b.top-height-12,ps:scale,dx:a.left-b.left+(a.width-84)/2,dy:a.bottom-b.top-height-110,ds:.42};};
// All sixteen slots surround the fire in two arcs, leaving the gong approach free.
var sanOldInstrument=drawInstrument;drawInstrument=function(i,t){if(PIXEL)return sanOldInstrument(i,t);};
function sanDrawAtlas(k,x,y,w){if(!nxInstrumentAtlas||!nxInstrumentAtlas.complete||!nxInstrumentAtlas.naturalWidth)return false;var i=NX_KITS.findIndex(function(a){return a.id===k;}),cw=nxInstrumentAtlas.naturalWidth/3,ch=nxInstrumentAtlas.naturalHeight/2;tg.save();tg.globalCompositeOperation='screen';tg.drawImage(nxInstrumentAtlas,(i%3)*cw,Math.floor(i/3)*ch,cw,ch,x-w/2,y-w*.43,w,w*.86);tg.restore();return true;}
var sanFire=drawFire;drawFire=function(t){sanFire(t);if(PIXEL)return;SAN.slots.forEach(function(k,i){var a=Math.PI*.08+(i%8)/7*Math.PI*.84,x=G.fire.x+Math.cos(a)*TWd*(i<8?.20:.30),y=G.fire.y+Math.sin(a)*THd*(i<8?.14:.25),w=Math.max(28,Math.min(66,TWd*.068));sanDrawAtlas(k,x,y,w);tg.fillStyle='#dbc9a0';tg.font='9px system-ui';tg.textAlign='center';tg.fillText(String(i+1).padStart(2,'0'),x,y+w*.5);hit('san-slot',x,y,w*.42,i);});};
var sanDrums=drawDrums;drawDrums=function(t){if(PIXEL)return sanDrums(t);for(var i=0;i<8;i++){var x=TWd*(.65+(i%4)*.058),y=THd*(.73+Math.floor(i/4)*.085),w=TWd*(i===0?.10:.062);sanDrawAtlas(SAN.kit,x,y,w);hit('san-drum',x,y,w*.42,i);}};
tc.addEventListener('pointerdown',function(e){var p=tPos(e),h=hitAt(p[0],p[1]);if(!h||!['san-slot','san-drum'].includes(h.k)||malletHit(p[0],p[1]))return;e.stopImmediatePropagation();var k=NX_KITS.find(function(k){return k.id===(h.k==='san-slot'?SAN.slots[h.d]:SAN.kit);});if(typeof ufItem==='function'&&h.k==='san-slot'){playInst(ufItem(k),ctx().currentTime+.01,0,.85);}else if(typeof UF!=='undefined'){playDrum(DRUMS[h.d%3],ctx().currentTime+.01,.85);}else playDrum({voice:k.voice,f:k.f,vol:.8},ctx().currentTime+.01,.85);beatImpulse();},{capture:true});
// A self-contained gong encounter: climb, transformation, then retreat into the sky.
var sanStrike=templeStrike;templeStrike=function(speed){if(PIXEL)return sanStrike(speed);var previous=startShow;startShow=function(){};try{sanStrike(speed);}finally{startShow=previous;}SAN.dragonAt=tClock;};
var sanGarg=drawGargoyle;drawGargoyle=function(t){if(PIXEL)return sanGarg(t);var age=tClock-SAN.dragonAt;if(age<0||age>15)return;if(!GARG.spr)prepAnimal(GARG);if(!DRAGON.spr)prepAnimal(DRAGON);var climb=smooth(age/4),flight=smooth((age-6)/9),r=tc.getBoundingClientRect(),b=bg.getBoundingClientRect(),x=r.left-b.left+G.gong.x+G.gong.R*(1-climb)+flight*W*.26,y=r.top-b.top+lerp(G.gy,G.gong.y-G.gong.R-18,climb)-flight*H*.56,sz=(70+30*smooth((age-4)/2))*(1-flight*.95),m=smooth((age-4)/2);go.save();go.translate(x,y);go.rotate(Math.sin(t*5)*.04*(1-flight));go.globalAlpha=(1-m)*(1-flight);go.drawImage(GARG.spr,-sz/2,-sz,sz,sz);go.globalAlpha=m*(1-flight);go.drawImage(DRAGON.spr,-sz/2,-sz,sz,sz);go.restore();};
root.dataset.sanctuaryRelease='r3';
/* Shared instrument catalogue and preferences, presentation-specific scenery. */
var UF={current:null,graphs:{},effects:{},edit:null,cloudUser:null,saveTimer:null};
var UF_PARAMS={Reverb:[['decay','Raumgröße',2.8,.2,6,.1],['tone','Höhen',5500,500,12000,100]],Delay:[['time','Zeit · s',.375,.05,1.5,.005],['feedback','Feedback',.3,0,.85,.01]],Chorus:[['rate','Rate · Hz',.8,.05,5,.05],['depth','Tiefe · ms',8,1,15,.5]],Flanger:[['rate','Rate · Hz',.18,.05,3,.05],['depth','Tiefe · ms',4,.5,6,.1]],Phaser:[['rate','Rate · Hz',.3,.05,5,.05],['frequency','Frequenz · Hz',600,120,2400,10]],Tremolo:[['rate','Rate · Hz',4,.1,12,.1],['depth','Tiefe',.5,0,1,.01]],Saturation:[['drive','Drive',2,1,8,.1],['tone','Höhen',8000,800,16000,100]],'Low-pass':[['frequency','Grenzfrequenz · Hz',2400,100,18000,50],['q','Resonanz',.7,.1,12,.1]],'High-pass':[['frequency','Grenzfrequenz · Hz',320,20,5000,10],['q','Resonanz',.7,.1,12,.1]],Compressor:[['threshold','Threshold · dB',-18,-48,0,1],['ratio','Ratio',3,1,12,.5]]};
NX_FX.forEach(function(n){UF.effects[n]={enabled:false,mix:.3,tracks:[],values:{}};UF_PARAMS[n].forEach(function(p){UF.effects[n].values[p[0]]=p[2];});});
var ufBaseKits=NX_KITS.slice();INSTR.slice(0,8).forEach(function(it,i){NX_KITS.push({id:it.id,name:it.name,voice:it.voice,f:hz(0,it.oct),ico:it.ico,original:it.id,atlasIndex:i});});
function ufItem(k){return INSTR.find(function(i){return i.id===(k.original||'garden-'+k.id);});}
function ufTracks(){return INSTR.concat(DRUMS.map(function(d){return {id:'drum:'+d.id,name:d.name,on:d.on};}));}
var ufAtlasImage=new Image();ufAtlasImage.src=nxAsset('instruments-orchestra.png');
var ufAtlas= sanAtlas;sanAtlas=function(id){var k=NX_KITS.find(function(k){return k.id===id;});if(!k||!k.original)return ufAtlas(id);if(k.atlasIndex==null)return sanPicture('Musik','#93c5b4');return '<span class="san-photo" style="background-image:url('+nxAsset('instruments-orchestra.png')+');background-size:400% 200%;background-position:'+((k.atlasIndex%4)*100/3)+'% '+(k.atlasIndex<4?0:100)+'%"></span>';};
var ufCanvasAtlas=sanDrawAtlas;sanDrawAtlas=function(id,x,y,w){var k=NX_KITS.find(function(k){return k.id===id;});if(!k||!k.original)return ufCanvasAtlas(id,x,y,w);if(k.atlasIndex==null){tg.save();tg.fillStyle='#c6d9bd';tg.font=Math.round(w*.6)+'px serif';tg.textAlign='center';tg.fillText('♫',x,y+w*.15);tg.restore();return true;}if(!ready(ufAtlasImage))return false;var cw=ufAtlasImage.naturalWidth/4,ch=ufAtlasImage.naturalHeight/2;tg.save();tg.globalCompositeOperation='screen';tg.drawImage(ufAtlasImage,(k.atlasIndex%4)*cw,Math.floor(k.atlasIndex/4)*ch,cw,ch,x-w/2,y-w*.5,w,w);tg.restore();return true;};
var ufPreview=nxInstrument;nxInstrument=function(k){if(k.original){var it=ufItem(k);playInst(it,ctx().currentTime+.01,0,.8);ST.sel=it.id;return;}ufPreview(k);};
SAN.slots=['guitar','flute','violin','cello','frame','djembe','bongo','asia','synth','vinyl','piano','chimes','guitar','frame','dj','bowl'].map(function(k){return k==='bowl'?'asia':k;});
function ufSnapshot(){return {slots:SAN.slots.slice(),kit:SAN.kit,effects:UF.effects,pattern:patternData(),bpm:TEMPO.bpm};}
var ufSaveLocal=sanSave;sanSave=function(){ufSaveLocal();if(!S.me)return;clearTimeout(UF.saveTimer);UF.saveTimer=setTimeout(function(){var studioPrefs=Object.assign({},S.studioPrefs||{},{sanctuary:ufSnapshot()});saveSettings({studio:studioPrefs}).then(function(r){if(r&&r.error)toast('Speichern nicht möglich · bitte erneut versuchen');else S.studioPrefs=studioPrefs;});},700);};
onState.push(function(){if(!S.me){UF.cloudUser=null;return;}if(UF.cloudUser===S.me.id)return;UF.cloudUser=S.me.id;var p=S.studioPrefs&&S.studioPrefs.sanctuary;if(!p)return;if(Array.isArray(p.slots)&&p.slots.length===16)SAN.slots=p.slots.map(function(id){return NX_KITS.some(function(k){return k.id===id;})?id:'frame';});if(ufBaseKits.some(function(k){return k.id===p.kit;}))sanKit(p.kit);if(p.effects)NX_FX.forEach(function(n){var e=p.effects[n];if(!e)return;UF.effects[n].enabled=!!e.enabled;UF.effects[n].mix=clamp(+e.mix||0,0,1);UF.effects[n].tracks=Array.isArray(e.tracks)?e.tracks.filter(function(id){return ufTracks().some(function(t){return t.id===id;});}):[];UF_PARAMS[n].forEach(function(x){UF.effects[n].values[x[0]]=clamp(Number(e.values&&e.values[x[0]])||x[2],x[3],x[4]);});});if(p.pattern){applyPattern({data:p.pattern,bpm:p.bpm||72});window.vrStudio.stop();}studioPaint();});
// Cache one audio chain per track; reroute existing voices as controls change.
function ufDisconnect(g){g.nodes.forEach(function(n){try{if(n.stop)n.stop();n.disconnect();}catch(e){}});g.input.disconnect();g.nodes=[];}
function ufGraph(id){var c=ctx(),g=UF.graphs[id];if(!g){g=UF.graphs[id]={input:c.createGain(),nodes:[]};ufBuild(g,id);}return g.input;}
function ufBuild(g,id){ufDisconnect(g);var c=ctx(),tail=g.input;function keep(n){g.nodes.push(n);return n;}
NX_FX.forEach(function(name){var f=UF.effects[name];if(!f.enabled||!f.tracks.includes(id))return;var input=tail,out=keep(c.createGain()),dry=keep(c.createGain()),wet=keep(c.createGain()),v=f.values;dry.gain.value=1-f.mix;wet.gain.value=f.mix;input.connect(dry);dry.connect(out);var node;
if(name==='Reverb'){node=keep(c.createConvolver());var length=Math.floor(c.sampleRate*v.decay),ir=c.createBuffer(2,length,c.sampleRate);for(var ch=0;ch<2;ch++){var data=ir.getChannelData(ch);for(var i=0;i<length;i++)data[i]=(Math.random()*2-1)*Math.pow(1-i/length,2.8);}node.buffer=ir;var tone=keep(c.createBiquadFilter());tone.frequency.value=v.tone;input.connect(node);node.connect(tone);tone.connect(wet);}
else if(name==='Delay'){node=keep(c.createDelay(2));node.delayTime.value=v.time;var fb=keep(c.createGain());fb.gain.value=v.feedback;input.connect(node);node.connect(fb);fb.connect(node);node.connect(wet);}
else if(['Chorus','Flanger','Phaser'].includes(name)){node=keep(name==='Phaser'?c.createBiquadFilter():c.createDelay(.1));var lfo=keep(c.createOscillator()),depth=keep(c.createGain());lfo.frequency.value=v.rate;if(name==='Phaser'){node.type='allpass';node.frequency.value=v.frequency;depth.gain.value=v.frequency*.65;lfo.connect(depth);depth.connect(node.frequency);}else{node.delayTime.value=name==='Chorus'?.025:.009;depth.gain.value=v.depth/1000;lfo.connect(depth);depth.connect(node.delayTime);}input.connect(node);node.connect(wet);lfo.start();}
else if(name==='Tremolo'){node=keep(c.createGain());node.gain.value=1-v.depth/2;var lfo=keep(c.createOscillator()),depth=keep(c.createGain());lfo.frequency.value=v.rate;depth.gain.value=v.depth/2;lfo.connect(depth);depth.connect(node.gain);input.connect(node);node.connect(wet);lfo.start();}
else if(name==='Saturation'){node=keep(c.createWaveShaper());var curve=new Float32Array(1024);for(var i=0;i<1024;i++)curve[i]=Math.tanh((i/511.5-1)*v.drive)/Math.tanh(v.drive);node.curve=curve;node.oversample='2x';var tone=keep(c.createBiquadFilter());tone.frequency.value=v.tone;input.connect(node);node.connect(tone);tone.connect(wet);}
else if(name==='Compressor'){node=keep(c.createDynamicsCompressor());node.threshold.value=v.threshold;node.ratio.value=v.ratio;input.connect(node);node.connect(wet);}
else{node=keep(c.createBiquadFilter());node.type=name==='Low-pass'?'lowpass':'highpass';node.frequency.value=v.frequency;node.Q.value=v.q;input.connect(node);node.connect(wet);}wet.connect(out);tail=out;});tail.connect(studioBus());}
function ufRebuild(){Object.keys(UF.graphs).forEach(function(id){ufBuild(UF.graphs[id],id);});}
var ufOut=voiceOut;voiceOut=function(dry,wet){if(PIXEL||!UF.current)return ufOut(dry,wet);var c=ctx(),g=c.createGain();g.gain.value=dry==null?1:dry;g.connect(ufGraph(UF.current));return g;};
var ufPlay=playInst;playInst=function(it,t,deg,vel){var before=UF.current;UF.current=it.id;try{ufPlay(it,t,deg,vel);}finally{UF.current=before;}};
var ufDrum=playDrum;playDrum=function(d,t,vel){var before=UF.current;UF.current='drum:'+(d.id||'low');try{ufDrum(d,t,vel);}finally{UF.current=before;}};
function ufEffectDialog(name){UF.edit=name;var f=UF.effects[name];openModal('<div class="uf-effect-title">'+sanPicture(name,SAN_COLORS[NX_FX.indexOf(name)%6])+'<div><small>KLANGALCHEMIE</small><h4>'+name+'</h4></div></div><label class="uf-enabled"><input type="checkbox" data-uf-enabled '+(f.enabled?'checked':'')+'> Effekt aktiv</label>'+UF_PARAMS[name].concat([['mix','Dry / Wet',.3,0,1,.01]]).map(function(p){var value=p[0]==='mix'?f.mix:f.values[p[0]];return '<label>'+p[1]+' <output>'+value+'</output><input data-uf-param="'+p[0]+'" type="range" aria-label="'+p[1]+'" min="'+p[3]+'" max="'+p[4]+'" step="'+p[5]+'" value="'+value+'"></label>';}).join('')+'<h4 style="margin-top:22px">Spuren zuordnen</h4><p class="vr-muted">Der Effekt wirkt nur auf ausgewählte Spuren.</p><div class="uf-track-list">'+ufTracks().map(function(t,i){return '<label><input type="checkbox" data-uf-track="'+esc(t.id)+'" '+(f.tracks.includes(t.id)?'checked':'')+'><span>'+String(i+1).padStart(2,'0')+'</span> '+esc(t.name)+'</label>';}).join('')+'</div><button data-uf-done>Fertig</button>',function(){mbox.querySelector('[data-uf-enabled]').onchange=function(e){f.enabled=e.target.checked;ufRebuild();studioPaint();sanSave();};mbox.querySelectorAll('[data-uf-param]').forEach(function(el){el.oninput=function(){var k=el.dataset.ufParam,value=+el.value;if(k==='mix')f.mix=value;else f.values[k]=value;el.parentElement.querySelector('output').textContent=value;};el.onchange=function(){ufRebuild();sanSave();};});mbox.querySelectorAll('[data-uf-track]').forEach(function(el){el.onchange=function(){var id=el.dataset.ufTrack;f.tracks=f.tracks.filter(function(t){return t!==id;});if(el.checked)f.tracks.push(id);ufRebuild();sanSave();};});mbox.querySelector('[data-uf-done]').onclick=function(){closeModal();studioPaint();};});}
if(!PIXEL)studio.addEventListener('click',function(e){var b=e.target.closest('[data-nx]');if(b&&b.dataset.nx.startsWith('fx-')){e.stopImmediatePropagation();ufEffectDialog(b.dataset.nx.slice(3));}},{capture:true});
var ufPaint=window.vrStudioPaint;window.vrStudioPaint=function(){ufPaint();if(PIXEL||!ST.open)return;studio.querySelectorAll('.nx-fx [data-nx]').forEach(function(b){var f=UF.effects[b.dataset.nx.slice(3)];b.classList.toggle('on',f.enabled);b.setAttribute('aria-pressed',String(f.enabled));});};
// Canvas interactions play the same samples and voices as their studio tracks.
tc.addEventListener('pointerdown',function(e){var p=tPos(e),h=hitAt(p[0],p[1]);if(!h||malletHit(p[0],p[1]))return;if(h.k==='san-slot'){e.stopImmediatePropagation();var k=NX_KITS.find(function(k){return k.id===SAN.slots[h.d];});var it=ufItem(k);if(it)playInst(it,ctx().currentTime+.01,0,.9);}if(h.k==='san-drum'){e.stopImmediatePropagation();playDrum(DRUMS[h.d%3],ctx().currentTime+.01,.8);}if(h.k==='uf-animal'){e.stopImmediatePropagation();nxDiscover(h.d);}},{capture:true});
// Install before prior capture handlers by marking their event targets handled below.
if(PIXEL){studioHtml=sanHTML;window.vrStudioPaint=sanPaint;}
if(!PIXEL){
 var wildlife=new Image();wildlife.src=nxAsset('wildlife-atlas.png');
 wildlife.onload=function(){ANIMALS.forEach(function(a,i){var tile=mk(384,384),g=tile.getContext('2d'),cw=wildlife.naturalWidth/6,ch=wildlife.naturalHeight/6;g.drawImage(wildlife,(i%6)*cw,Math.floor(i/6)*ch,cw,ch,0,0,384,384);tile.complete=true;tile.naturalWidth=384;tile.naturalHeight=384;a.photo=tile;});};
 var ufPrep=prepAnimal;prepAnimal=function(a){var photo=a.photo;ufPrep(a);if(photo&&photo.tagName==='CANVAS')a.photo=photo;};
 // Keep a clear semicircle around the fire; animals arrive along the outer garden.
 drawGathered=function(t){if(!NX.animals||!ready(wildlife))return;gathered.forEach(function(g,i){var idx=ANIMALS.indexOf(g.A),a=VR_ANIMALS[idx];if(idx<0)return;var age=t-g.t0,k=smooth(age/4.5),x,y,w=Math.min(105,TWd*.09)*(SIZE[g.A.n]||1);if(a.h==='fly'){x=TWd*(.16+(i%6)*.135);y=THd*(.24+(i%2)*.06);w*=.7;}else if(a.h==='water'){x=TWd*(.07+(i%3)*.035);y=THd*(.55+(i%2)*.06);w*=.6;}else{x=TWd*(i%2===0?.12+(i%3)*.075:.81+(i%3)*.045);y=THd*(.68+(i%4)*.064);}x=lerp(i%2?-w:TWd+w,x,k);var cw=wildlife.naturalWidth/6,ch=wildlife.naturalHeight/6;tg.save();tg.globalCompositeOperation='screen';tg.globalAlpha=k;tg.drawImage(wildlife,(idx%6)*cw,Math.floor(idx/6)*ch,cw,ch,x-w/2,y-w/2,w,w);tg.restore();hit('uf-animal',x,y,w*.43,g.A.n);});};
 // Replace the duplicate discovered-animal row with the single spatial collection.
 drawFire=function(t){var old=G.s;G.s*=1.4;nxGatherFire(t);G.s=old;SAN.slots.forEach(function(k,i){var left=i<8,j=i%8,x=TWd*(left?.23+j%4*.077:.52+j%4*.065),y=THd*(left?.87+Math.floor(j/4)*.082:.88+Math.floor(j/4)*.08),w=Math.min(75,TWd*.066);sanDrawAtlas(k,x,y,w);tg.fillStyle='#dbc9a0';tg.font='9px system-ui';tg.textAlign='center';tg.fillText(String(i+1).padStart(2,'0'),x,y+w*.43);hit('san-slot',x,y,w*.42,i);});};
 drawDrums=function(t){var plan=[[.64,.76,1.5],[.70,.79,1],[.75,.75,.8],[.79,.80,.85],[.68,.84,.7],[.75,.86,.7],[.82,.87,.55],[.85,.77,.65]];if(SAN.kit==='kit'){sanDrawAtlas('kit',TWd*.73,THd*.78,TWd*.23);for(var i=0;i<6;i++)hit('san-drum',TWd*(.66+i%3*.055),THd*(.73+Math.floor(i/3)*.10),TWd*.029,i);return;}plan.forEach(function(p,i){var kind=SAN.kit==='frame'?['frame','frame','bongo','frame','frame','bongo','frame','frame'][i]:SAN.kit;var w=TWd*.065*p[2];sanDrawAtlas(kind,TWd*p[0],THd*p[1],w);hit('san-drum',TWd*p[0],THd*p[1],w*.4,i);});};
 // Static rooted tree, soft optical edges and continuous parallax; no strip reveal.
 var forestLayer=new Image();forestLayer.src=nxAsset('forest-foreground.png');var rootedTree=new Image();rootedTree.src=nxAsset('ancient-tree.png');
 drawFerns=function(t){var depth=smooth((svS-.4)/1.7)*NX.plants;gf.save();gf.globalCompositeOperation='screen';if(ready(rootedTree)){var h=H*.95,w=h*rootedTree.naturalWidth/rootedTree.naturalHeight;gf.globalAlpha=depth*.67;gf.translate(-w*.2+(reduced?0:Math.sin(t*.08)*6),H);gf.rotate(reduced?0:Math.sin(t*.16)*.003);gf.drawImage(rootedTree,0,-h,w,h);}gf.restore();if(ready(forestLayer)){var h=H*.68,w=h*forestLayer.naturalWidth/forestLayer.naturalHeight;[-1,1].forEach(function(side){gf.save();gf.globalCompositeOperation='screen';gf.globalAlpha=depth*.55;gf.translate(side<0?-w*.4:W+w*.4,H+40*(1-depth));gf.scale(side<0?1:-1,1);gf.rotate(reduced?0:Math.sin(t*.25+side)*.008);gf.drawImage(forestLayer,0,-h,w,h);gf.restore();});}};
 var ufEnergy=drawEnergy;drawEnergy=function(t){ufEnergy(t);var alpha=1-smooth(svS/1.8);if(alpha<.01)return;ge.save();ge.globalAlpha=alpha*.72;var cx=W*.5,cy=H*.42,scale=Math.min(W,H)*.38;ge.translate(cx,cy);ge.scale(1,.54);ge.rotate(-.25);for(var i=0;i<7;i++){var r=scale*(.22+i*.115);ge.strokeStyle='rgba(108,180,210,.13)';ge.lineWidth=.8;ge.beginPath();ge.arc(0,0,r,0,Math.PI*2);ge.stroke();var a=t*(.045/(1+i*.3))+i*1.87,x=Math.cos(a)*r,y=Math.sin(a)*r;var glow=ge.createRadialGradient(x-2,y-3,0,x,y,5+i*.9);glow.addColorStop(0,['#e8d1a0','#7cdfdf','#b9d88c','#e6a889','#c3aedc','#d1b38a','#9ebfdb'][i]);glow.addColorStop(1,'#142335');ge.fillStyle=glow;ge.beginPath();ge.arc(x,y,5+i*.9,0,7);ge.fill();}var sun=ge.createRadialGradient(0,0,0,0,0,45);sun.addColorStop(0,'#fff0bd');sun.addColorStop(.2,'#e8bd67');sun.addColorStop(1,'#d99c3100');ge.fillStyle=sun;ge.fillRect(-45,-45,90,90);ge.restore();};
 // True column width is shared by layout and docking; never clip a wider aside.
 var toggle=$('vr-aside-toggle');toggle.textContent='Mixer & Sammlung';toggle.setAttribute('aria-expanded','false');root.classList.add('uf-aside-closed');
 toggle.onclick=function(){var closed=root.classList.toggle('uf-aside-closed');aside.classList.toggle('open',!closed);toggle.setAttribute('aria-expanded',String(!closed));toggle.textContent=closed?'Mixer & Sammlung':'Mixer schließen';resize();tResize();};
 var closeAside=document.createElement('button');closeAside.className='uf-close-aside';closeAside.textContent='Mixer schließen ×';closeAside.onclick=toggle.onclick;aside.prepend(closeAside);
 var ufResize=new ResizeObserver(function(){resize();tResize();});ufResize.observe($('vr-main'));
 window.vrDockC=function(pw,ph,C){var open=!root.classList.contains('uf-aside-closed'),a=aside.getBoundingClientRect(),b=bg.getBoundingClientRect();var available=open?a.width-28:Math.min(300,W*.34),scale=Math.min(available/pw,.65,(H*.36)/ph),height=ph*scale;adock.style.height='0px';aside.style.paddingBottom=(height+150)+'px';return {px:open?a.left-b.left+14:W-pw*scale-18,py:H-height-22,ps:scale,dx:open?a.left-b.left+(a.width-72)/2:W-95,dy:H-height-110,ds:.36};};
 // Smoothly enter the existing bottom garden, preserving a natural upward journey.
 var back=document.createElement('a');back.className='uf-entrance-link';back.href='./';back.textContent='← Welten wählen';$('vr-ui').appendChild(back);
 if(modeQuery.has('entrance')){history.scrollRestoration='manual';root.classList.add('uf-entering');requestAnimationFrame(function(){tSec.scrollIntoView({block:'start'});resize();tResize();requestAnimationFrame(function(){root.classList.remove('uf-entering');});});}
}
// Each new catalogue sample can provide a photo; Pixel derives a tiny raster sprite locally.
var ufSampleImages={};onState.push(function(){S.samples.filter(function(s){return s.kind!=='song';}).forEach(function(sample){if(INSTR.some(function(i){return i.id==='sample-'+sample.id;}))return;var it=nxTrack('sample-'+sample.id,sample.name,'pluck');it.sample=sample;NX_KITS.push({id:it.id,name:it.name,voice:'pluck',f:220,ico:'♫',original:it.id,sampleId:sample.id});if(safeUrl(sample.thumb_url)){var im=new Image();im.src=sample.thumb_url;ufSampleImages[it.id]=im;}assignSample(it,sample);});});
var ufAllAtlas=sanDrawAtlas;sanDrawAtlas=function(id,x,y,w){if(ufSampleImages[id]){var im=ufSampleImages[id];if(!ready(im))return false;tg.save();tg.globalCompositeOperation='screen';tg.drawImage(im,x-w/2,y-w/2,w,w);tg.restore();return true;}return ufAllAtlas(id,x,y,w);};
var ufAllHTML=sanAtlas;sanAtlas=function(id){if(ufSampleImages[id])return '<span class="san-photo" style="background-image:url('+esc(ufSampleImages[id].src)+');background-size:contain;background-repeat:no-repeat;background-position:center"></span>';return ufAllHTML(id);};
root.dataset.sanctuaryRelease='r4';
