...
var PIXEL = true;   // Pixel-Art-Fassung: alles gezeichnet, keine Fotos, weniger Last
...
$('vr-full').onclick = function () {
  ...
  fsWin = window.open(VR_CONFIG.standalone, 'neo-visual-room');
...
function tResize() {
  tDPR = Math.min(window.devicePixelRatio || 1, 1.5); TWd = tSec.clientWidth; THd = tSec.clientHeight;
...
function drawTemple(t, show) {
  var T = G.temple, s = G.s * 1.1, winter = seasonAmt(4), pa = 0, ph = THd * .62;
...
if ('IntersectionObserver' in window) new IntersectionObserver(function (e) { tVisible = e[0].isIntersecting; if (tVisible) { tResize(); loadShowAssets(); } }).observe(tSec);
else tVisible = true;
